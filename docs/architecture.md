...
A shell loads and saves UTF-8 Documents, debounces edits, invokes the Core renderer off the UI thread, and replaces Preview content on the UI thread. It retains the last successful Preview if rendering fails. Cross-platform document-title semantics are specified in [Document title conventions](document-titles.md); Apple retains native `DocumentGroup` title and edited-state presentation. Android keeps interactive editing out of whole-document Compose state: its Piece Tree structurally shares unchanged text, its custom View lays out only visible lines, and Save streams a snapshot. The decision and performance contract are recorded in [ADR 0001](adr/0001-android-incremental-virtualized-editor.md) and the [benchmark guide](benchmarks.md).
