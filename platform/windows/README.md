...
Release builds use the static MSVC runtime and force `/APPCONTAINER:NO`; Windows App SDK bootstrap auto-initialization remains enabled by the pinned NuGet package. CI verifies the PE is not AppContainer-enabled, rejects PDB/import-library leakage and setup files at or above 5 MiB, then installs, launches, waits for a surviving window, and uninstalls the app. On failure it prints setup and Windows Application Event logs.
